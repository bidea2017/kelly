...
// Second is ThrottleConfig to use with this policy. For further information on options,
// see ThrottleConfig further above.
func Throttle(quota *Quota, options ...*ThrottleConfig) ServeHTTP {
	...
	return func(w http.ResponseWriter, r *http.Request) (*http.Request, bool) {
		id := makeKey(o.KeyPrefix, quota.KeyId(), o.Identify(r))

		if controller.DeniesAccess(id) {
			msg := newAccessMessage(o.StatusCode, o.Message)
			setRateLimitHeaders(w, controller, id)
...
func newOptions(options []*ThrottleConfig) *ThrottleConfig {
	o := ThrottleConfig{
